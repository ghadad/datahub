    ...
    <div class="columns">
      <div class="rules-list column is-3">
        ...
        <table class="table is-fullwidth is-dark">
          ...
          <tr v-show="rules.length==0">
            <th class="has-text-centered"><button class="button is-primary is-large" @click="displayFastAssign()">Fast
                mapping assignment</button></th>
          </tr>
          <draggable id="rules-table " v-model="cRules" tag="tbody">
            <tr v-for="(rule,index) in cRules" :key="rule.name" class="clickable"
              ...
              <td @click="activeRule=rule;activeIndex=index;newActiveIndex=null">{{ rule.name||rule.goTo}}</td>
              <td @click="activeRule=rule;activeIndex=index;newActiveIndex=null">{{ index+1 }}</td>
            </tr>
          </draggable>
        </table>
      </div>
      <div v-show="fastAssignment" class="column is-9">
        <div class="columns">
          <div class="column is-3">
            <table class="fast-mapping table is-bordered is-fullwidth">
            <thead><th>Collector properties</th></thead>
              <tbody>
                <draggable v-model="collectorProperties" group="a1">
                  <tr v-for="(p,index) in collectorProperties" :key="index" class="clickable">
                    <td> {{p}}</td> <td><b-icon icon="arrow-right"></b-icon></td>
                  </tr>
                </draggable>
              </tbody>
            </table>
          ...
          <div class="column is-3">
            <table class="fast-mapping table is-bordered is-fullwidth" id="mapping-target">
            <thead><th>go to Entity  properties</th></thead>
              <tbody>
                <draggable v-model="fastMappingIndex" group="a">
                  <tr v-for="(p,index) in fastMappingIndex" :key="index" class="">
                    <td> {{p.name}}</td>  
                  </tr>
                </draggable>
              </tbody>
          ...
          <div class="column is-2">
           <table class="fast-mapping table is-bordered is-fullwidth">
           <thead><th>Entity properties</th></thead>
              <tbody>
            <draggable v-model="entityProperties" group="a">
              <tr v-for="(p,index) in entityProperties" :key="index" class="clickable">
             <td>   {{p.name}} </td>
              </tr>
            </draggable>
             </tbody>
                    ...
                    <div class="column is-2">
   <button class="button is-info"> Generate rules</button>
          </div>
        </div>
      </div>
        ...
        <div class="columns">
          ...
          <div class="column is-3">
            <div class="is-normal buttons-group">
              ...
              <button class="button is-danger is-small" v-show="activeIndex>=0 && dStep==1" @click="delRule(2)">
                Are you
                sure ?
              </button>
            </div>
          ...
          <div class="column">
            ...
            <button v-show="displayState=='edit' && allCollapse==true" class="clickable" @click="display('expand')"
              title="Add new mapping rule after this position !">
              <b-icon class="is-pulled-right clickable" icon="chevron-down" size="is-small" type="is-dark"></b-icon>
            </button>
            <button v-show="displayState=='edit' && allCollapse==false" class="clickable" @click="display('collapse')"
              title="Add new mapping rule after this position !">
              <b-icon class="is-pulled-right clickable" icon="chevron-up" size="is-small" type="is-dark"></b-icon>
            </button>
            ...
            <button v-show="displayState!='list'" class="clickable" @click="display('list')"
              title="Add new mapping rule after this position !">
              <b-icon class="is-pulled-right clickable" icon="list" size="is-small" type="is-primary"></b-icon>
            </button>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      displayFastAssign() {
        this.collectorProperties = this.$_.cloneDeep(this.collector.properties);
...
<style scoped>
  ...
  #mapping-target {}
  .fast-mapping.table td {width:100%}
</style>