...
exports = module.exports = class Flow {
  ...
  saveErrorBulk(data, stage = "noStage", err, returnError = false) {
    ...
    data = data.map(function (doc) {
      ...
      doc.entityId = self.flowConfig.targetEntity;
      doc.errorCreated = __app.ts();
      delete doc._id;
      delete doc._rev;
  ...
  saveError(data, stage = "noStage", err, returnError = false) {
    ...
    __app.couchDb.upsert(this.errorCollection, data).then(res => console.log(res)).catch(e => console.error(e));
  }
  ...
  async init() {
    if (!this.flowConfig.targetEntity) {
      throw new __app.error("Please suply a taretEntity ")
    }
  ...
  async exec() {
    ...
    await this.validate();
    let flowStatus = await self.collector.collect(this.runFlow(self), this.runFlowBulkUpsert(self), 13, 100);
    __app.logger.info("Interface end ! ", flowStatus)
  }
  ...
  runFlow(self) {
    return async function (data, returnObjectOnStage = false) {
      ...
      let oldData = {};
      try {
        oldData = await self.getExists(self.flowConfig.targetEntity, newData._id);
      } catch (e) {
        //return self.saveError(newData, "getKey", e, returnObjectOnStage);
      }
      ...
      try {
        ...
        if (e.code == "DROP") {
          ...
          return self.setError(newData, "mapping", e);
        }
