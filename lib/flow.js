...
exports = module.exports = class Flow {
  ...
  saveError(data, stage = "noStage", err, returnError = false) {
    __app.logger.error("Flow error on stage:", stage, err.message, err.code ? "code:" + err.code : "")
    data.$error = {
      stage: stage,
      message: err.message
    };
  ...
  runFlow(self) {
    return async function (data, returnObjectOnStage = false) {
      ...
      try {
        ...
        newData = self.saveError(data, "collector", e, returnObjectOnStage);
      }
      ...
      try {
        ...
        newData = self.saveError(newData, "getKey", e, returnObjectOnStage);
      }
      ...
      try {
        ...
        mappedData = self.saveError(newData, "mapping", e, returnObjectOnStage);
      }
      ...
      try {
        ...
        mappedData = self.saveError(mappedData, "mappingHandler", e, returnObjectOnStage);
      }
      ...
      try {
        ...
        mappedData = self.saveError(mappedData, "discover", e, returnObjectOnStage);
      }
      ...
      try {
        ...
        mappedData = self.saveError(mappedData, "discoverHandler", e, returnObjectOnStage);
      }
      ...
      // on teset mode return fullfilled object
      if (returnObjectOnStage)
        return mappedData;
      ...
      try {
        ...
        mappedData = self.saveError(mappedData, "saveTarget", e, returnObjectOnStage);
      }
      return mappedData;
    }
  }
