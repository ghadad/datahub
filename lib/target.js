...
exports = module.exports = class Terget {
  ...
  async saveBulk(db, data, options = {}) {
    ...
    if (self.flowConfig.revisionsCollection) {
      ...
      let ids = data.filter(d => d.$control.change && d.$control.patch).map(d => {
        return d._id
      });
      ...
      let patches = data.filter(d => d.$control.change && d.$control.patch).map(d => {
        let existVersions = __app.lodash.get(existsDocsByKey[d._id] || {}, 'versions', []);
        existVersions.push(d.$control);
        return {
          _id: d._id,
          versions: existVersions
        };
      });

      await __app.couchDb.upsertBulk(this.flowConfig.revisionsCollection, patches, options)
    }
    return data;
  }
