...
global.__app = {
  ...
  public: path.resolve(__base, "ui/public"),
  examples: path.resolve(__base, "examples"),
  confDir: path.resolve(__base, "config"),
  secureDir: path.resolve(__base, ".secure/"),
  moment: moment,
  json: json5,
  config: {},
...
__app.secureConf = require(__app.secureDir);

__app.mailer = nodemailer.createTransport(__app.secureConf.mailer);
...
async function bootstrap(args) {
  ...
  __app.sysDb.connect(__app.config.couchSysDb);

  __app.local.dbs = await __app.couchDb.getAll("databases");

  Object.assign(__app.config, await __app.sysDb.get("config", __app.args.env), {
    loaded: true
  });
  console.log(__app.config)
  return true;
}
